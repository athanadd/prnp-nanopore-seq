...
mkdir -p $DIR/sniffles_out
files=` ls $bamdir | grep '\_sorted_mdfix.bam$' `
...
sniffles --genotype --report_seq --report_read_strands -s 100 -r $min_read_length --min_length $sniffles_min_var_length -t $thrds -m $bamdir/$file -v $DIR/sniffles_out/${filename}_sniffles.vcf
}
